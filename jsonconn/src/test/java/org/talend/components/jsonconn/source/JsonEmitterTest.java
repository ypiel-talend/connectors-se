...
class JsonEmitterTest extends AbstractTest {

    @EnvironmentalTest
    void testFd() {
        InputStream in = getClass().getClassLoader().getResourceAsStream("fd.json");
        final String json = isToString(in);
        config.getDataset().setJson(json);

        final String configStr = configurationByExample().forInstance(config).configured().toQueryString();

        Job.components() //
                .component("emitter", "Demo://jsonInput?" + configStr) //
    ...
    @EnvironmentalTest
    void testInput() {
        InputStream in = getClass().getClassLoader().getResourceAsStream("corona-api.countries.json");
        final String json = isToString(in);
        config.getDataset().setJson(json);
    ...
    void testDoubleArray() {
        InputStream in = getClass().getClassLoader().getResourceAsStream("doubleArray.json");
        final String json = isToString(in);
        config.getDataset().setJson(json);
