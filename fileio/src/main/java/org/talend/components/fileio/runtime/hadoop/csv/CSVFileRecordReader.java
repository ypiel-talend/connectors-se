package org.talend.components.fileio.runtime.hadoop.csv;
...
public class CSVFileRecordReader extends RecordReader<LongWritable, BytesWritable> {
    ...
    public CSVFileRecordReader(String recordDelimiter, String encoding, Character textEnclosure, Character escapeChar)
    ...
    public boolean nextKeyValue() throws IOException {
        if (!isComplexCSV || (textEnclosure == null)) {// escape char also can escape newLine?not common, textEnclosure is more
                                                       // common for that case.
            boolean hasNext = next();

            if (hasNext) {
                // TODO not use value.getBytes() directly, as not sure value.getLength() == value.getBytes().length, will use
                // copyBytes
        ...
        do {
            ...
            if (hasNext) {
                ...
                String currentSubline = new String(java.util.Arrays.copyOfRange(value.getBytes(), 0, value.getLength()),
                        encoding);

                // Get the number of escape character on the current
                // substring,
                // in order to check if we have or not a complete column
                for (int index = currentSubline.indexOf(textEnclosure); index >= 0; index = currentSubline.indexOf(textEnclosure,
                        index + 1)) {
