 ...
 */
public abstract class UgiFileSourceBase<K, V, SourceT extends UgiFileSourceBase<K, V, SourceT>>
        extends RelaxedHDFSFileSource<K, V> {
    ...
    /* Override with UGI if available. */
    @Override
    public final List<? extends BoundedSource<KV<K, V>>> split(final long desiredBundleSizeBytes, final PipelineOptions options)
            throws Exception {
    ...
    protected static class UgiFileReader<K, V> extends RelaxedHDFSFileReader<K, V> {
        ...
        public UgiFileReader(UgiFileSourceBase<K, V, ?> source) throws IOException {
            super(source, source.filepattern, source.formatClass,
                    source.serializableSplit == null ? null : source.serializableSplit.getSplit());
            this.source = source;
