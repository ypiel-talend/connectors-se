...
public class CSVFileInputFormat extends org.apache.hadoop.mapreduce.lib.input.FileInputFormat<LongWritable, BytesWritable> {
    ...
    private CSVFileRecordReader createRecordReader(final String rowDelimiter, final String encoding,
            final Character textEnclosure, final Character escapeChar) throws IOException {
            ...
            if (length != 0) {
                ...
                if (isSplitable(job, path)) {
                    ...
                    while (((double) bytesRemaining) / splitSize > SPLIT_SLOP) {
                        ...
                        splits.add(makeSplit(path, offset, splitSize, splitIndex++, blkLocations[blkIndex].getHosts(),
                    ...
                    if (bytesRemaining != 0) {
                        long offset = length - bytesRemaining;
                        int blkIndex = getBlockIndex(blkLocations, offset);
                        splits.add(makeSplit(path, offset, bytesRemaining, splitIndex++, blkLocations[blkIndex].getHosts(),
                                blkLocations[blkIndex].getCachedHosts()));
                    }
                } else { // not splitable
                    splits.add(makeSplit(path, skipLength, length - skipLength, splitIndex++, blkLocations[0].getHosts(),
