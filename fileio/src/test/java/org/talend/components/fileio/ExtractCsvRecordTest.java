package org.talend.components.fileio;
...
public class ExtractCsvRecordTest {
    ...
    /** The DoFn under test. */
    // no quote, no escape
    private final DoFnTester<BytesWritable, IndexedRecord> fnBasic = DoFnTester
            .of(new SimpleRecordFormatCsvIO.ExtractCsvRecord(';', false, "UTF-8", null, null));

    // quote : ", no escape char
    private final DoFnTester<BytesWritable, IndexedRecord> fnQuote = DoFnTester
            .of(new SimpleRecordFormatCsvIO.ExtractCsvRecord(';', false, "UTF-8", '\"', null));

    // quote : ", escape char : \
    private final DoFnTester<BytesWritable, IndexedRecord> fnQuoteAndEscape = DoFnTester
            .of(new SimpleRecordFormatCsvIO.ExtractCsvRecord(';', false, "UTF-8", '\"', '\\'));

    // encoding only
    private final DoFnTester<BytesWritable, IndexedRecord> fnEncoding = DoFnTester
            .of(new SimpleRecordFormatCsvIO.ExtractCsvRecord(';', false, "GBK", null, null));
    ...
    public void testEncoding() throws Exception {
        ...
        assertThat("encoding", toArray(fnEncoding.processBundle(bsw).get(0)),
                arrayContaining(new String[] { "王伟", "北京", "Talend" }));
