...
import org.talend.components.recordprovider.conf.Dataset;
import org.talend.components.recordprovider.conf.Datastore;
...
@WithComponents(value = "org.talend.components.recordprovider")
class GenericEmitterTest {
    ...
    @EnvironmentalTest
    void testFixedRecordSplit() {
        config.setOverwriteDataset(true);
        config.getCodingConfig().setProvider(CodingConfig.RECORD_TYPE.FIXED);
        config.getCodingConfig().setNbRecord(50);
        config.setSplits(3);
        final List<Record> records = getRecords();
    ...
    private List<Record> getRecords() {
        ...
        Job.components() //
                .component("emitter", "Generator://recordprovider?" + configStr) //
