...
public class QueryBuilderInsert implements QueryBuilder {

    private final TableNameDataset dataSet;
    ...
    public String buildQuery(final List<Record> records) {
        ...
        return queries.computeIfAbsent(entries.stream().map(Schema.Entry::getName).collect(joining("::")), key -> {
            ...
            final StringBuilder query = new StringBuilder("INSERT INTO ").append(dataSet.getTableName());
